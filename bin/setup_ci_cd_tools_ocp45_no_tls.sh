...
./setup_nexus3.sh admin $NEXUS_PASSWORD http://$(oc get route nexus --template='{{ .spec.host }}') ${CICD_NEXUS_USER} ${CICD_NEXUS_PASSWORD}
echo "expose port 5000 for container registry"
oc expose dc nexus --port=5000 --name=nexus-registry
oc expose svc nexus-registry --port=5000
